...
const DEEPSEEK_API_KEY =
  process.env.DEEPSEEK_API_KEY || "sk-2ccda9689c7e4810a3237d8881437f0e";
...
export class DeepseekAPI {
  ...
  private async getFeatureFlags(): Promise<FeatureFlags> {
    ...
    if (
      this.featureFlags &&
      Date.now() - this.lastFlagsFetch < this.FLAG_CACHE_TTL
    ) {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a senior product manager with 10 years of experience, combined with expertise in technical writing and task management.",
          },
